...
def rand_query(kelompok:int):
    if kelompok == 1:
        return choice((TAMBAH,)*0 + (RESIGN,)*0 + (CARRY,)*0 + (SIMULASI,)*1)
    elif kelompok == 3:
        return choice((TAMBAH,)*1 + (RESIGN,)*1 + (SEBAR,)*1)
    else:
        raise Exception()

TIPE_PANGKAT = DUA_PANGKAT_BERBEDA
...
def solve(input_seluruhnya):  # use input() and print()
    ...
    if kelompok_tc == 1:  # TRCS
        solve_kelompok_1(banyak_node, pangkat, edges, queries)
    elif kelompok_tc == 2:  # Networking
        solve_kelompok_2(banyak_node, pangkat, edges, queries)
    elif kelompok_tc == 3:  # Sebar
        solve_kelompok_3(banyak_node, pangkat, edges, queries)
    elif kelompok_tc == 4:
        solve_kelompok_4(banyak_node, pangkat, edges, queries)
...
def generate_soal():  # use randint() (or other random func) and print()
    ...
    if tc_count < 20:
        ...
        banyak_node = randint(2, 14)
        banyak_edge_awal = randint(1, max(1, min(16, banyak_node*(banyak_node-1) // 5)))

    if TIPE_PANGKAT == PANGKAT_UNIK:
        pangkat = [i for i in range(1, banyak_node+1)]
        shuffle(pangkat)
    elif TIPE_PANGKAT == DUA_PANGKAT_BERBEDA:
        a,b = sample([i for i in range(1, banyak_node+1)]*2, 2)
        pangkat = [choice((a,b)) for i in range(banyak_node)]
    elif TIPE_PANGKAT == PANGKAT_ACAK:
        pangkat = [randint(1, banyak_node) for i in range(banyak_node)]
    else:
        raise Exception()

    if kelompok_tc == 1:
        generate_soal_kelompok_1(banyak_node, pangkat, banyak_edge_awal)
    elif kelompok_tc == 2:
        generate_soal_kelompok_2(banyak_node, pangkat, banyak_edge_awal)
    elif kelompok_tc == 3:
        generate_soal_kelompok_3(banyak_node, pangkat, banyak_edge_awal)
    elif kelompok_tc == 4:
        generate_soal_kelompok_4(banyak_node, pangkat, banyak_edge_awal)
    else:
        raise Exception()

def generate_soal_kelompok_1(banyak_node, pangkat, banyak_edge):
    if tc_count < 20:
        banyak_query = randint(99_000, 100_000)
    else:
        banyak_query = randint(3, 15)
