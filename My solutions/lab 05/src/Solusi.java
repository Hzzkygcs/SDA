...
//    public static PrintStream out = System.out;
    ...
    public static HashMap<Long, AVLTree<Kotak>> tipe_tipe = new HashMap<>(32);
    
    public static boolean tipe_maks_8 = true;
    ...
    public static boolean kotak_in_range(KotakSingleTipe kotak, long L, long R){
        return L <= kotak.harga && kotak.harga <= R;
    }
    
    // TODO
    static void handleBeli(int L, int R){
        if (tipe_maks_8){
            AVLTree<KotakSingleTipe> sorter = new AVLTree<>();
            Iterator<Long> keys = tipe_tipe.keySet().iterator();
            
            KotakSingleTipe pasangan_kiri_1 = null;
            KotakSingleTipe pasangan_kanan_1 = null;
            KotakSingleTipe pasangan_kiri_2 = null;
            KotakSingleTipe pasangan_kanan_2 = null;
            
            while (keys.hasNext()){
                long key = keys.next();
                AVLTree<Kotak> temp_tree = tipe_tipe.get(key);
                Kotak temp_node = temp_tree.lower_boundary(temp_tree.root, new Kotak(L, null));
                Kotak temp_node2 = temp_tree.upper_boundary(temp_tree.root, new Kotak(R, null));
                
                if (temp_node != null) {
                    KotakSingleTipe temp = new KotakSingleTipe(temp_node.harga, key);
                    if (kotak_in_range(temp, L, R))
                            sorter.root = sorter.insert(sorter.root, temp);
                }
                if (temp_node2 != null) {
                    KotakSingleTipe temp = new KotakSingleTipe(temp_node2.harga, key);
                    if (kotak_in_range(temp, L, R))
                        sorter.root = sorter.insert(sorter.root, temp);
                }
            }
            
            List<KotakSingleTipe> list = new ArrayList<>(20);
            sorter.to_list(sorter.root, list);
            ...
            pasangan_kiri_1 = list.get(0);
            if (pasangan_kiri_1.harga > R){
                out.println("-1 -1");
                return;
            }
            
            for (int i = list.size()-1; i > 0; i--) {
                if (list.get(i).tipe_permen != pasangan_kiri_1.tipe_permen
                    && kotak_in_range(list.get(i), L, R)){
                    pasangan_kanan_1 = list.get(i);
                    break;
                }
            }
            
            
            pasangan_kanan_2 = list.get(list.size()-1);
            if (pasangan_kanan_2.harga < L){
                out.println("-1 -1");
                return;
            }
            for (int i = 1; i < list.size()-1; i++) {
                if (list.get(i).tipe_permen != pasangan_kanan_2.tipe_permen
                    && kotak_in_range(list.get(i), L, R)){
                    pasangan_kiri_2 = list.get(i);
                    break;
                }
            }
            
            if ((pasangan_kiri_1 == null || pasangan_kanan_1 == null) && (pasangan_kiri_2 == null)){
                out.println("-1 -1");
                return;
            }
            if (pasangan_kiri_1 == null || pasangan_kanan_1 == null){
                out.print(pasangan_kiri_2.harga);
                out.print(' ');
                out.println(pasangan_kanan_2.harga);
                return;
            }
            
            if (pasangan_kiri_2 == null || (pasangan_kanan_1.harga - pasangan_kiri_1.harga) > (pasangan_kanan_2.harga - pasangan_kiri_2.harga)){
                out.print(pasangan_kiri_1.harga);
                out.print(' ');
                out.println(pasangan_kanan_1.harga);
                return;
            }
            
            // kalau selisih 2 >= selisih 1. Harus output pasangan 2 (sekalipun sama dengan) karena output yang jumlahnya terbesar
            out.print(pasangan_kiri_2.harga);
            out.print(' ');
            out.println(pasangan_kanan_2.harga);
    ...
    static void handleStock(String nama, int harga, int tipe){
        ...
        if (tipe_maks_8){
            long tipe_long = tipe;
            if (!tipe_tipe.containsKey(tipe_long)){
                tipe_tipe.put((long) tipe, new AVLTree<>());
                if (tipe_tipe.size() > 8)
                    tipe_maks_8 = false;
            }
            AVLTree<Kotak> avl = tipe_tipe.get(tipe_long);
            kotak = new Kotak(harga, null);
            avl.root = avl.insert(avl.root, kotak);
            node = avl.get(avl.root, kotak);
            kotak.node_ini = node;
            kotak = node.key;
            permen = new Permen(tipe, 0);
            result = kotak.tipe_permen.get(kotak.tipe_permen.root, permen);
    ...
    static void handleSoldOut(String nama){
        ...
        if (tipe_maks_8){
            harga = tipe_from_name.get(nama).harga;
            tipe = tipe_from_name.get(nama).tipe;
    
            AVLTree<Kotak> avl = tipe_tipe.get(tipe);
            
            kotak = new Kotak(harga, null);
            kotak_node = avl.get(avl.root, kotak);
            kotak = kotak_node.key;
            permen = kotak.tipe_permen.get(kotak.tipe_permen.root, new Permen(tipe)).key;
            permen.jumlah -= 1;
            if (permen.jumlah == 0){
                kotak.tipe_permen.root = kotak.tipe_permen.deleteNode(kotak.tipe_permen.root, permen);
        
                if (kotak.tipe_permen.root == null){
                    avl.root = avl.deleteNode(avl.root, kotak);
                    
                    if (avl.root == null){
                        tipe_tipe.remove(tipe);
                    }
                }
            }
        }
...
class KotakSingleTipe implements Comparable<KotakSingleTipe> {
    long harga;
    long tipe_permen;
    
    
    KotakSingleTipe(long harga, long tipe){
        this.harga = harga;
        tipe_permen = tipe;
    }
    
    @Override
    public int compareTo(KotakSingleTipe o) {
        if (harga != o.harga)
            return Long.compare(harga, o.harga);
        return Long.compare(tipe_permen, o.tipe_permen);
    }
    
    @Override
    public String toString() {
        return "KS:" + harga + "." + tipe_permen;
    }
}
