...
namespace JavaMiniParser
{
    internal class Program {
        public static void Main(string[] args){
            string src_code_path = @"D:\01 Kuliah\01 Dokumen\71 - Struktur Data Algoritma\08 TP\TP_01\src\Solusi.java";
            string source_code = System.IO.File.ReadAllText(src_code_path);

            JavaMiniParser java_mini_parser = new JavaMiniParser(source_code);
            // Console.WriteLine(java_mini_parser.parse());
            java_mini_parser.parse();
            
            // Console.WriteLine(java_mini_parser.get_public_class_name());

            /*var match_collection = java_mini_parser.get_assigned_variable_declarations();
            foreach (var variabel in match_collection){
            ...
            match_collection = java_mini_parser.get_assigned_static_variable_declarations();
            foreach (var variabel in match_collection){
                Console.WriteLine(variabel);
            }
            ...
            foreach (var class_declaration in class_declarations){
                Console.WriteLine(class_declaration.name);
            ...
            foreach (var o in java_mini_parser.tokenized_splitted){
                if (o is string)
                    Console.WriteLine("{0}{1}", "string: ", o);
                else if (o is ClassDeclaration)
                    Console.WriteLine("{0}{1}", "classdec: ", o);
                else if (o is AssignedVariableDeclaration)
                    Console.WriteLine("{0}{1}", "assg dec: ", o);
            }
        }
    }
    ...
    abstract class VariableDeclaration : Declaration
    {
        ...
        public VariableDeclaration(string name, string type, string type_generic = "",
        ...
        public override string ToString(){
            return String.Format("{0}{1}{2} {3}", base.ToString(),  type, type_generic, name);
        }
    }
    
    
    class AssignedVariableDeclaration : VariableDeclaration
    {
        public string assigned_value;
        
        public AssignedVariableDeclaration(string assigned_value, string name, string type, 
                ...
                base(name, type, type_generic, visibility_modifier, static_abstract, is_synchronized, is_final)
        {
    ...
    class ClassDeclaration : Declaration
    {
        ...
        public ClassDeclaration(string name, string generic, VisibilityModifier visibility_modifier=VisibilityModifier.DEFAULT, 
    ...
    class JavaMiniParser{
        ...
        static readonly Regex GET_PUBLIC_CLASS_NAME_REGEX = 
            new Regex("public\\s+class\\s+([a-zA-Z0-9_][a-zA-Z_]*)\\s*{");


        private static readonly string _KEYWORD_MODIFIERS =
            "(?:(public|private|protected)\\s+)?(?:(abstract|static)\\s+)?(?:(final)\\s+)?(?:(synchronized)\\s+)?";
        
        static readonly Regex GET_CLASS_DECLARATION_REGEX = 
            new Regex(_KEYWORD_MODIFIERS + "class\\s+([a-zA-Z0-9_]+)\\s*(<[^>]*>)?\\s*(?:extends\\s+([a-zA-Z0-9_]+)\\s*(<[^>]*>)?)?\\s*(?:implements\\s+([^{]+))?\\s*\\{");
        
        static readonly Regex GET_DECLARED_ASSIGNED_REGEX = 
            new Regex(_KEYWORD_MODIFIERS + "([a-zA-Z0-9_]+)\\s*(<[a-zA-Z0-9_,\\s]*>)?\\s+([a-zA-Z0-9_]+)\\s*=(?!=)([^;]+);");

        private static readonly Regex GET_MAIN_METHOD_REGEX =
        ...
        private static readonly int VARIABLE_TYPE_GROUP = 5;
        private static readonly int GENERIC_GROUP = 6;
        private static readonly int VARIABLE_NAME_GROUP = 7;
        private static readonly int ASSIGNED_VALUE_GROUP = 8;
        ...
        private Stack<Tuple<string, string>> token_stack = new Stack<Tuple<string, string>>();  // item1: token, item2: the actual content
        ...
        public static ClassDeclaration get_class_declaration_from_match(Match match){
            ...
            return new ClassDeclaration(
                ...
                StaticAbstractExtension.from_string(match.Groups[STATIC_ABSTRACT_GROUP].Value), match.Groups[CLASS_PARENT_GROUP].Value,
        ...
        public static AssignedVariableDeclaration get_assigned_variable_declaration_from_match(Match match){
            ...
            return new AssignedVariableDeclaration(match.Groups[ASSIGNED_VALUE_GROUP].Value,
                ...
                match.Groups[GENERIC_GROUP].Value, visibility, static_abstract,
        ...
        public List<ClassDeclaration> get_class_declarations(){
            ...
            while (declared_variable_ptr < declared_variables.Count){
                if (!declared_variables[declared_variable_ptr].Success) continue;
                prev_class_obj.variable_declarations.Add(
                    get_assigned_variable_declaration_from_match(declared_variables[declared_variable_ptr]));
                declared_variable_ptr++;
            }
        ...
        // di dalamnya
        public void parse_tokenized_splitted(){  // must be called after parse()
